...
int main( int , char** )
{
	WNDCLASSEXW wc = { sizeof( wc ) , CS_CLASSDC , WndProc , 0L , 0L , GetModuleHandle( nullptr ) , nullptr , nullptr , nullptr , nullptr , L"CookieMenuSandbox" , nullptr };
	::RegisterClassExW( &wc );
	HWND hwnd = ::CreateWindowW( wc.lpszClassName , L"Cookie Menu Sandbox (Synthetic Demo)" , WS_POPUP , 0 , 0 , 1920 , 1080 , nullptr , nullptr , wc.hInstance , nullptr );
	...
	while ( !done )
	{
		...
		if ( var->c_dpi.dpi_changed )
		{
			...
			io.Fonts->Clear();
			const float scale = var->c_dpi.dpi;
			set->c_font.inter_medium[0] = io.Fonts->AddFontFromMemoryTTF( inter_medium , sizeof( inter_medium ) , 15.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.inter_medium[1] = io.Fonts->AddFontFromMemoryTTF( inter_medium , sizeof( inter_medium ) , 16.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[0] = io.Fonts->AddFontFromMemoryTTF( icon , sizeof( icon ) , 14.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[1] = io.Fonts->AddFontFromMemoryTTF( icon , sizeof( icon ) , 16.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[2] = io.Fonts->AddFontFromMemoryTTF( icon , sizeof( icon ) , 40.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[3] = io.Fonts->AddFontFromMemoryTTF( icon2 , sizeof( icon2 ) , 15.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[4] = io.Fonts->AddFontFromMemoryTTF( icon2 , sizeof( icon2 ) , 9.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[5] = io.Fonts->AddFontFromMemoryTTF( icon2 , sizeof( icon2 ) , 76.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.icon[6] = io.Fonts->AddFontFromMemoryTTF( icon2 , sizeof( icon2 ) , 96.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
			set->c_font.name = io.Fonts->AddFontFromMemoryTTF( inter_medium , sizeof( inter_medium ) , 18.f * scale , &cfg , io.Fonts->GetGlyphRangesCyrillic() );
