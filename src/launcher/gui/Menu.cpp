...
{
	WNDCLASSEX wc = { sizeof(WNDCLASSEX), CS_CLASSDC, Menu::WndProc, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, _T("CookieInjector"), NULL };
	::RegisterClassEx(&wc);
	this->hwnd = ::CreateWindow(wc.lpszClassName, _T("Cookie Injector"),
...
void Menu::loop()
{
	...
	while (this->isMenuOn)
	{
		...
		ImGui::PopStyleColor();
		ImGui::Text("DLL: %ls %s", vars::str_embedded_dll_name.data(), COOKIE_VERSION);
		if (ImGui::Button("Patch VAC3"))
		{
			if(!this->isPatchingVac)
			{
				injlog::info("Patch VAC3 started");
		...
		if (ImGui::Button("Inject"))
		{
			if (!this->isInjecting)
			{
				injlog::info("Inject started");
				std::thread(&Injector::injectThreadEntry).detach();
			}
		}
		if (this->isPatchingVac)
		{
