...
auto WINAPI CCrashLog::VectoredExceptionHandler( PEXCEPTION_POINTERS pExceptionInfo )->LONG
{
    ...
    if ( ExceptionCode == STATUS_ACCESS_VIOLATION ||
        ...
        ExceptionCode == STATUS_STACK_OVERFLOW ||
         ...
         ExceptionCode == STATUS_HEAP_CORRUPTION ||
         ExceptionCode == STATUS_INVALID_PARAMETER ||
         CPP_EH_EXCEPTION )
#else
    if ( ExceptionCode == STATUS_ACCESS_VIOLATION ||
        ExceptionCode == STATUS_STACK_BUFFER_OVERRUN ||
...
auto CCrashLog::OpenCrashLogFile() -> void
{
    std::lock_guard lk( m_Lock );

    if ( m_hCrashLogFile != INVALID_HANDLE_VALUE )
        return;
    ...
    const auto path = dir + L"\\" + GetCurrentDateTime() + L".log";

    m_hCrashLogFile = CreateFileW(
        path.c_str() ,
        GENERIC_WRITE ,
        FILE_SHARE_READ ,
        nullptr ,
...
auto CCrashLog::CloseCrashLogFile() -> void
{
    std::lock_guard lk( m_Lock );
    ...
    FlushFileBuffers( m_hCrashLogFile );
    CloseHandle( m_hCrashLogFile );
    m_hCrashLogFile = INVALID_HANDLE_VALUE;
}
...
auto CCrashLog::WriteCrashLogFile( const char* fmt , ... ) -> void
{
    ...
    {
        DWORD written = 0;
        WriteFile( m_hCrashLogFile , buff , lstrlenA( buff ) , &written , nullptr );
    }
}
