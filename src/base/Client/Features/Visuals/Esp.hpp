...
namespace ESP
{
    ...
    inline bool skipPlayerRendering = false;
    ...
    inline void Render()
    {
        ...
        if (localPawn)
        {
            ...
            if (config.maxDistance > 0.0f || config.bDistance)
        ...
        const float maxDistSq = config.maxDistance > 0.0f ? config.maxDistance * config.maxDistance : 0.0f;
            ...
            if (bomb)
            {
                ...
                if (bombNode)
                {
                    ...
                    if (!bombOrigin.IsZero())
                    {
                        ...
                        if (Game::WorldToScreenCached(bOrg, bX, bY, screenW, screenH))
                        {
        ...
        if ( !skipPlayerRendering && localPawn )
        for (int i = 1; i <= 64; i++) // Usually limits to max players for controller scan
        {
            ...
            if (maxDistSq > 0.0f && !localOrigin.IsZero())
            {
            ...
            if (!Game::WorldToScreenCached(org, orgX, orgY, screenW, screenH))
                continue;
            ...
            if (!Game::WorldToScreenCached(head, headX, headY, screenW, screenH)) continue;
            ...
            if (config.bDistance && !localOrigin.IsZero())
            {
