...
//   3. Press F10/F11 to cycle 鈥?watch your weapon change
...
namespace CustomTexture
{
    ...
    // -------------------------------------------------------
    // Load a custom image from disk 鈫?D3D11 texture + SRV
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // PSSetShaderResources hook 鈥?the hot path
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // Init 鈥?hook PSSetShaderResources via vtable swap
    // -------------------------------------------------------
    ...
    // -------------------------------------------------------
    // Shutdown 鈥?restore vtable, release resources
    // -------------------------------------------------------
    ...
    inline void UpdateHookState(ID3D11DeviceContext* context)
    {
        if (!context)
            return;

        if (enabled && imageLoaded && !hooked)
            Init(context);
        else if (hooked && (!enabled || !imageLoaded))
            Shutdown();
    }
