...
namespace SkinChanger
{
    ...
    // Global knife setting 鈥?which knife model to use
    ...
    // Static attribute buffers 鈥?allocated ONCE each, reused every apply.
    inline CEconItemAttribute* g_attrBuffer = nullptr;      // for weapons


    inline void CreateAttributes(uintptr_t item, int paintKit, int seed, float wear, int statTrak = -1)
    {
        ...
        const int attrCount = statTrak >= 0 ? 5 : 3;

        if (!g_attrBuffer)
        {
            g_attrBuffer = reinterpret_cast<CEconItemAttribute*>(
                VirtualAlloc(nullptr, sizeof(CEconItemAttribute) * 5,
        ...
        if (statTrak >= 0)
        {
            g_attrBuffer[3] = MakeAttribute(80, static_cast<float>(statTrak));
            g_attrBuffer[4] = MakeAttribute(81, 0.f);
        }

        CPtrGameVector newList;
        newList.size = attrCount;
        newList.ptr = reinterpret_cast<uintptr_t>(g_attrBuffer);
        Game::Write<CPtrGameVector>(attrListAddr, newList);
    }

    inline void ApplyMeshGroup(uintptr_t weapon, uintptr_t localPawn) noexcept
    {
        auto* weaponEnt = reinterpret_cast<C_CSWeaponBase*>(weapon);
        if (!weaponEnt)
            return;

        if (auto* sceneNode = weaponEnt->m_pGameSceneNode())
            sceneNode->SetMeshGroupMask(2);

        if (auto* localEntity = reinterpret_cast<C_CSPlayerPawn*>(localPawn))
        {
            if (auto* viewModel = localEntity->GetViewModel())
            {
                if (auto* vmNode = viewModel->m_pGameSceneNode())
                    vmNode->SetMeshGroupMask(2);
            }
        }
    }
    ...
    inline void RemoveAttributes(uintptr_t item)
    {
        ...
        // Zero the vector 鈥?the buffer stays valid and allocated
    ...
    inline void InitRegen()
    {
        ...
        regenAddr = Game::SigScan(
            ...
            Patterns::sig_RegenerateWeaponSkins
        );
    ...
    inline void ApplyAndRegen(uintptr_t weapon, const SkinConfig& skin, uint16_t defIndex)
    {
        ...
        CreateAttributes(item, skin.paintKit, skin.seed, skin.wear, skin.statTrak);

        printf("[DBG] Step4: calling regen\n");
        CallRegen();

        printf("[DBG] Step5: FULL RESET 鈥?restoring weapon to clean state\n");

        // RESET EVERYTHING 鈥?make weapon look untouched to game's destructor
    ...
    inline bool NeedsTick()
    {
        if (forceUpdate.load() || knifeChangerEnabled)
            return true;

        for (const auto& [_, cfg] : weaponSkins)
        {
    ...
    inline void TickInner()
    {
        ...
        {
            (void)inventory;
            if ( SkinChangerInventory::UsesInventoryPath() )
            {
                if ( forceUpdate.load() )
        ...
        {
            if (lastAppliedWeapon != 0)
                printf("[DBG] DEAD: lifeState=%d hp=%d 鈥?resetting tracking\n", lifeState, health);
            lastAppliedWeapon = 0;
            lastAppliedKit = 0;
            return;
        }
        ...
        // ===== WEAPON SKINS =====
        uintptr_t activeWeapon = Game::GetActiveWeapon(localPawn);
        if (activeWeapon)
        {
            ...
            // ===== KNIFE MODEL CHANGER =====
            // Write EVERY TICK 鈥?the game continuously resets these values
            ...
            if (knifeChangerEnabled && (isDefaultKnife || isOurSpoofedKnife) && targetKnifeDef > 0)
            {
                ...
                // KEY: Write SubclassID 鈥?this is what actually changes the 3D model in Source 2
                ...
                if ( auto* weaponEnt = reinterpret_cast<C_CSWeaponBase*>( activeWeapon ) )
                {
                    if ( auto* sceneNode = weaponEnt->m_pGameSceneNode() )
                        sceneNode->SetMeshGroupMask( 2 );
                ...
                if (it != weaponSkins.end() && it->second.enabled && it->second.paintKit > 0)
                {
                    ...
                    if (needsApply)
                    {
                        ...
                        if (lifeState == 0 && health > 0)
                        {
                            ...
                            ticksSinceApply = 0;
                            ApplyMeshGroup(activeWeapon, localPawn);
                        }
                    }
                }
            }
        ...
        // Chams are handled by the Chams module (DrawObject hook in scenesystem.dll).
        (void)chamsConfig;
