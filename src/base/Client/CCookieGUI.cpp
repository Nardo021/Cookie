...
auto CCookieGUI::OnInit( IDXGISwapChain* pSwapChain ) -> void
{
	DXGI_SWAP_CHAIN_DESC SwapChainDesc;

	if ( FAILED( pSwapChain->GetDevice( IID_PPV_ARGS( &m_pDevice ) ) ) )
	{
		DEV_LOG( "[error] CCookieGUI::OnInit: #1\n" );
		return;
	}
	...
	{
		DEV_LOG( "[error] CCookieGUI::OnInit: #2\n" );
		return;
	}
...
auto CCookieGUI::OnDestroy() -> void
{
...
void CCookieGUI::OnResizeBuffers( IDXGISwapChain* pSwapChain )
{
	UNREFERENCED_PARAMETER( pSwapChain );

	ClearRenderTargetView();
	m_needRecreateDeviceObjects = true;

	if ( m_bInit )
		ImGui_ImplDX11_InvalidateDeviceObjects();
}
 
void CCookieGUI::OnRender( IDXGISwapChain* pSwapChain )
{
...
LRESULT WINAPI CCookieGUI::GUI_WndProc( HWND hwnd , UINT uMsg , WPARAM wParam , LPARAM lParam )
{
	if ( uMsg == WM_CLOSE )
	{
		GetCookieClient()->OnShutdown();
		if ( GetCookieGUI()->m_bInit )
			GetCookieGUI()->OnDestroy();
		GetDllLauncher()->OnDestroy();
		return CallWindowProcA( GetCookieGUI()->m_WndProc_o , hwnd , uMsg , wParam , lParam );
	}
...
auto CCookieGUI::SetIndigoStyle() -> void
{
...
auto CCookieGUI::SetVermillionStyle() -> void
{
	auto& style = ImGui::GetStyle();
	auto& colors = style.Colors;
...
auto CCookieGUI::SetClassicSteamStyle() -> void
{
...
auto CCookieGUI::UpdateStyle() -> void
{
	ImGui::SetCurrentContext( m_pImGuiContext );

	if ( Settings::Menu::MenuStyle == EGuiStyle_t::GUI_STYLE_INDIGO )
		SetIndigoStyle();
	else if ( Settings::Menu::MenuStyle == EGuiStyle_t::GUI_STYLE_VERMILLION )
		SetVermillionStyle();
	else if ( Settings::Menu::MenuStyle == EGuiStyle_t::GUI_STYLE_CLASSIC_STEAM )
		SetClassicSteamStyle();
}

bool CCookieGUI::FreeTypeBuild::PreNewFrame()
{
	if ( !WantRebuild )
		return false;
...
auto CCookieGUI::ClearRenderTargetView() -> void
{
	if ( m_pRenderTargetView )
	{
