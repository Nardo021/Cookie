...
namespace Game
{
    ...
    // Entity identity stride 鈥?0x70 for current CS2 build (confirmed via memory scan)
    ...
    inline uintptr_t GetActiveWeapon(uintptr_t pawn)
    {
        ...
        const uint32_t handle = Read<uint32_t>(weaponServices + Offsets::m_hActiveWeapon);
    ...
    inline uintptr_t ResolveRelativeAddress(uintptr_t instruction, int offset, int size)
    {
        if (!instruction) return 0;
        int32_t rel = Read<int32_t>(instruction + offset);
        return instruction + size + rel;
    }

    // Scan function prologue for RIP-relative mov/lea and return resolved address
    inline uintptr_t FindRipTarget(uintptr_t func, size_t scanLen, uint8_t modrmReg = 0xFF)
    {
        if (!func) return 0;

        auto matchesReg = [modrmReg](uint8_t modrm) {
            if (modrmReg == 0xFF) return true;
            return ((modrm >> 3) & 7) == (modrmReg & 7);
        };
        ...
        for (size_t i = 0; i + 7 < scanLen; ++i)
        {
            ...
            // 48 8B / 48 8D with modrm [rip+disp32]
            if (p[0] == 0x48 && (p[1] == 0x8B || p[1] == 0x8D) && (p[2] & 0xC7) == 0x05 && matchesReg(p[2]))
                return ResolveRelativeAddress(func + i, 3, 7);

            // 4C 8B with REX.R
            if (p[0] == 0x4C && p[1] == 0x8B && (p[2] & 0xC7) == 0x05 && matchesReg(p[2]))
                return ResolveRelativeAddress(func + i, 3, 7);
        }
        return 0;
    }
    ...
    inline ViewMatrix g_cachedViewMatrix{};
    inline bool g_viewMatrixCached = false;

    inline void BeginFrameRender()
    {
        if (!clientBase)
        {
            g_viewMatrixCached = false;
            return;
        }

        g_cachedViewMatrix = Read<ViewMatrix>(clientBase + Offsets::dwViewMatrix);
        g_viewMatrixCached = true;
    }

    inline bool WorldToScreenCached(const float* worldPos, float& outX, float& outY, float screenW, float screenH)
    {
        if (!g_viewMatrixCached)
            return false;

        const ViewMatrix& vm = g_cachedViewMatrix;
