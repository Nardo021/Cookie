...
bool load_button( std::string_view label , bool* callback )
{
	...
	*state = ImClamp( *state + ( gui->fixed_speed( 8.f ) * ( !*callback ? 1.f : -1.f ) ) , 0.f , 1.f );
	...
	draw->render_text( window->DrawList , set->c_font.inter_medium[1] , rect.Min , rect.Max , gui->get_clr( clr->c_text.text ) , "UNLOAD" , nullptr , nullptr , ImVec2( 0.5f , 0.5f ) );

	draw->add_rect_filled( window->DrawList , rect.Min , rect.Max , gui->get_clr( clr->c_other_clr.accent_clr , *state ) , SCALE( 2.f ) );
	draw->render_text( window->DrawList , set->c_font.inter_medium[1] , rect.Min , rect.Max , gui->get_clr( clr->c_text.text_active , *state ) , "LOAD" , nullptr , nullptr , ImVec2( 0.5f , 0.5f ) );
...
bool c_widget::lua_tool_button( std::string_view icon , std::string_view aid )
{
	...
	*state = ImLerp( *state , hovered ? clr->c_text.text_active : clr->c_text.text_hov , gui->fixed_speed( 12.f ) );
	...
	draw->render_text( window->DrawList , set->c_font.icon[3] , rect.Min , rect.Max , gui->get_clr( *state ) , icon.data() , nullptr , nullptr , ImVec2( 0.5f , 0.5f ) );
...
bool c_widget::lua_selectable( lua_data* data , int lua_id )
{
	...
	const bool pressed = lua_tool_button( "B" , std::to_string( id ) );
