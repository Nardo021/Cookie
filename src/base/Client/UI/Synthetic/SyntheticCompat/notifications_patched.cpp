...
void c_notify::render_notify(int notification_index, float notification_alpha, float notification_offset, float notification_duration, float notification_delay, std::string_view notification_text, notify_position notification_position)
{
...

    {
        ...
        { // decorations
            ...
            draw->render_text(draw_list, font, { position.x + SCALE(10), position.y }, { position.x + CalcTextSize(notification_text.data()).x + SCALE(60), position.y + SCALE(40) }, gui->get_clr(clr->c_text.text_active), notification_text.data(), NULL, NULL, { 0.0, 0.5 }, NULL);
