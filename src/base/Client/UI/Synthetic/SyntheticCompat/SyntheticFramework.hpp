...
#include <framework/settings/elements.h>
...
#define SYNTHETIC_PI 3.14159265359f
...
#include <Client/UI/Synthetic/SyntheticCompat/lua_data_fwd.hpp>

inline D3DX11_IMAGE_LOAD_INFO g_dx11ImageInfo;
...
public:
    ...
    bool                            dropdown(std::string_view label, int* current_item, const std::vector<std::string>& items, int max_count);

    bool                            tool_dropdown(std::string_view label, int* current_item, const std::vector<std::string>& items, int max_count);
    
    void                            multi_dropdown(std::string_view label, bool variable[], const std::vector<std::string>& labels, int max_count);
...
public:
    ...
    float ease_in_elastic(float t) { const float c4 = (2 * SYNTHETIC_PI) / 2;  return (t <= 0.01f) ? 0.0f : (t >= 0.60f) ? 1.0f : pow(2, -10 * t) * sin((t * 10 - 0.75) * c4) + 1; }
